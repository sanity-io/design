...
function getNewState(state: LocationState, params: Partial<LocationState>) {
  return {
    path: params.path || '/',
...
export function LocationProvider(props: LocationProviderProps) {
  ...
  const pushState = useCallback((params: Partial<LocationState>) => {
  ...
  const replaceState = useCallback((params: Partial<LocationState>) => {
    const newState = getNewState(stateRef.current, params)
    const url = getUrlFromState(newState)
