...
import {createColorTheme, ThemeColorSchemes} from '../lib/theme'
import {multiply, screen} from './helpers'

const NEUTRAL_TONES = ['default', 'transparent']

export function _createColor(options: {color: ColorPalette}): ThemeColorSchemes {
  const {color} = options
  ...
  return createColorTheme({
    ...
    card: ({base, dark, muted, name, solid, state}) => {
      ...
      const bg: string = base.bg
      const skeletonFrom = mix(base.bg, tints[dark ? 950 : 100].hex)
    ...
    input: ({base, dark, mode, state}) => {
      ...
      if (state === 'hovered') {
        return {
          bg: base.bg,
          fg: base.fg,
          border: mix(base.bg, hues.gray[dark ? 800 : 300].hex),
      ...
      if (state === 'readOnly') {
        return {
          ...
          border: mix(base.bg, hues.gray[dark ? 900 : 200].hex),
          placeholder: mix(base.bg, hues.gray[dark ? 600 : 400].hex),
        }
      }
    ...
    syntax: ({base, dark}) => {
      ...
      const secondaryShade = dark ? 700 : 300
