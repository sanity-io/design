...
import {OutputOptions, rollup as _rollupBundle, watch as rollupWatch} from 'rollup'
...
export async function rollupBundle(opts: {
  ...
  const {build, cwd, input, tsconfig = 'tsconfig.json'} = opts
  const {format, target} = build
